...
public class HorarioFragment extends Fragment implements
	...
	private void initialiseTabHost(Bundle args) {
		...
		HorarioFragment.addTab((MenuEstudiante) getActivity(), this.mTabHost,
	...
	private static void addTab(Activity activity, TabHost tabHost,
		...
		tabInfo.fragment = ((MenuEstudiante)activity).getSupportFragmentManager()
				.findFragmentByTag(tag);
		if (tabInfo.fragment != null && !tabInfo.fragment.isDetached()) {
			FragmentTransaction ft = ((MenuEstudiante)activity).getSupportFragmentManager()
					.beginTransaction();
			ft.detach(tabInfo.fragment);
			ft.commit();
			((MenuEstudiante)activity).getSupportFragmentManager().executePendingTransactions();
		}
